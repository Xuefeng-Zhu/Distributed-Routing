...
class LearningSwitch (api.Entity):
    ...
    def handle_link_down(self, port):
        ...
        """
        for address, pt in self.route_table.items():
            if pt == port:
                del self.route_table[address]
                break
    ...
    def handle_rx(self, packet, in_port):
        ...
        if packet.src not in self.route_table:
            self.route_table[packet.src] = in_port
        out_port = self.route_table.get(packet.dst)
